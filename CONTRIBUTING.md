...
変更ごとに先に Issue を作成し、`issue/<番号>` ブランチを作ります。許可する PR の向きは `.github/branch-policy.toml` の `[[routes]]` で定義します。既定では `issue/<番号> -> main` です。別の統合・安定化ブランチが必要なプロジェクトだけ、追加の経路を明示的に設定してください。作業ブランチを最新化するときは上流ブランチを merge して通常 push します。`main` と追加した保護ブランチへの直接 push、force-push、削除は禁止です。マージ方式は rebase のみとし、コミットメッセージを PR タイトルによって置き換えません。Issue ブランチの PR 本文には対応する `Closes #<番号>` を単独行で記載してください。
