...
- Link every change Issue with its own `Closes #123` line. Put the Issue encoded in the human work branch first, then add any other Issues the same Pull Request closes; keep every line in the opening `Issue` section. Use the repository's non-closing release keyword, such as `Tracks #123`, for a release Pull Request merged before publication, so the Release Issue stays open. A release Pull Request merged after publication, such as one that records the release evidence, links the Release Issue with `Closes #123`.
...
- If a preflight fails, do not publish a ready Pull Request or diagnose the
  ...
  unresolved failure.
- Do not update the governing Issue's Project Status when the Pull Request opens. The trusted `Policy (Project)` workflow owns PR-observable transitions and sets `In review` for ready Pull Requests; it also handles draft transitions, synchronize, and unmerged close. The Project's built-in Item closed workflow moves a closed Issue to `Done`, and its built-in Item reopened workflow restores `Backlog` on reopen.
