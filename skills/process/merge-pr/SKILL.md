...
- Confirm the Pull Request is linked to the governing Issue. For human change work, require the Issue number encoded in the `issue/<number>` branch and the opening `Closes #<number>` reference. A release Pull Request merged before publication uses `Tracks #<number>` and must not close the Release Issue. A release Pull Request merged after publication, such as one that records the release evidence, uses `Closes #<number>`.
