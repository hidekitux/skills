...
2. Verify the Pull Request is merge-ready: approved review outcome, no blocking or unresolved findings, required checks, branch freshness, permissions, and repository merge policy.
...
- Do not merge a Pull Request that is missing its governing Issue, has scope drift, has unresolved review findings, or is still awaiting required review or checks. Route missing implementation or review work to `implement-issue`, `create-pr`, or `review-pr` instead of compensating here.
...
- Treat approval as explicit evidence from the repository's required reviewers or review policy. A comment, a dismissed approval, an approval from an insufficiently privileged reviewer, or an approval that became stale after synchronization is not sufficient.
