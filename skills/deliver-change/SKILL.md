...
4. Open and review the change with `create-pr` and `review-pr`, reworking until the review loop terminates.
...
- Preserve approval boundaries: `create-pr` opens the Pull Request only after user confirmation, and `implement-issue` edits only in-scope files from the verified plan.
...
- **External mutation authority:** only `create-pr` creates the Pull Request, pausing for user confirmation. `implement-issue` edits only files inside the verified plan's in-scope boundary.
