...
func CalculateTraceMetrics(traces []trace.Trace) TraceMetrics {
	...
	for _, item := range traces {
		...
		if item.Deliberation != nil {
			metrics.DeliberationCount++
			metrics.CandidateCount += len(item.Deliberation.Candidates)
			if item.Deliberation.MarginalCost != nil {
				metrics.MarginalElapsedMillis += item.Deliberation.MarginalCost.ElapsedMillis
				metrics.MarginalRetries += item.Deliberation.MarginalCost.Retries
			}
		}
