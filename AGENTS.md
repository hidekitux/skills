...
- Keep every per-skill asset, a file that belongs to exactly one skill, below its skill root. The evaluation inputs are the one exception: scenarios live in `evaluations/scenarios/<skill-name>/` and fixtures in `evaluations/fixtures/<key>/`. Installation copies the whole skill directory, so an evaluation input there would ship to every user and reach the agent under evaluation, which could then read a scenario's expectations. `check-evaluation` fails when a scenario directory, a scenario's `skill` field, or a cataloged skill loses that link.
...
- Write every artifact that stays in the repository in English: code comments, `docs/`, `SKILL.md`, commit message bodies, Issue and Pull Request bodies, and Pull Request review comments. Conversation with the user is not an artifact, and it is held in Japanese.
