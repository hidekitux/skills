...
| `check:repository` (twenty-five repository checks, `cmd/check-repository`) | 1 | every PR / `main` push / local | repository owner | ~0.3s warm | blocking |
...
| `test:all` (`cmd/test-go`, `go test -count=1 ./...` against a Git sentinel repository) | 1 | every PR / `main` push | repository owner | ~46s to ~54s; the test cache is off | blocking |
