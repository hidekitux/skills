...
`docs/`. Two tiers divide the 19 skills by where that prose goes. Tier A prose
...
| B | `analyze-codebase`, `analyze-project`, `retrospect-work`, `triage-issues`, `audit-workflow-enforcement`, `debug-code`, `write-tests`, `refactor-code`, `merge-pr`, `deliver-change`, `improve-project`, `resolve-defect` | None. The handoff report is the only prose, and it stays in the conversation |
...
The evidence for a tier is the skill's own artifact. For the 16 skills the
