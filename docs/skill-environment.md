...
The Provisioner runs `scripts/setup/run-mise.sh run setup:refresh` synchronously before execution. A local host opts into the worktrunk provider with `Provisioner.WithLocalWorktree()`. That provider uses `wt switch` with structured output and suppressed lifecycle hooks, then adopts the verified branch-owned path; a missing `wt` command selects the native-Git provider. The refresh path runs once after creation or reuse, so execution does not depend on a background checkout hook. Continuous integration leaves the opt-in unset and uses native Git directly. A missing command, malformed structured result, or non-zero setup result stops the selected skill. Use `wt list --format=json` for local inspection when available; native `git worktree list --porcelain` remains the machine-readable fallback.
