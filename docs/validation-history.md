...
the check that flagged it, so the search cannot count local catches. Of the four
matching commits, three change their commands: `3ee5c95` and `1afb081`
(`verify-fsl`) and `9b825f2` (`check-sensitive-content`). The fourth,
`aa5d81d`, edits the `check-public-status` entry in
`docs/validation-tiers.md`. The search misses `1ef692a` (a `lint-commits`
test) and `042f6ba` (the `policy.yml` jobs), which fix failures this record
cites from run logs.
