...
- A task must describe an action. The approved verb vocabulary is `check`, `diagnose`, `evaluate`, `format`, `generate`, `install`, `lint`, `mutate`, `publish`, `setup`, `test`, `validate`, and `verify`.
