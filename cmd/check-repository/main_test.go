...
func TestRunRepositoryChecksAgainstRepositoryRoot(t *testing.T) {
	...
	want := "all 8 repository checks passed"
	if !strings.Contains(out.String(), want) {
		t.Fatalf("output missing %q:\n%s", want, out.String())
	}
	for _, label := range []string{"[validate-repository]", "[check-tool-licenses]", "[validate-script-tests]", "[check-sensitive-content]", "[check-mutation-badges]", "[check-analyze-readonly]", "[check-guided-paths]", "[check-catalog-docs]"} {
