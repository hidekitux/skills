...
func TestRunRepositoryChecksAgainstRepositoryRoot(t *testing.T) {
	...
	for _, label := range []string{"[validate-repository]", "[check-tool-licenses]", "[validate-script-tests]", "[check-sensitive-content]", "[check-mutation-badges]", "[check-mutation-triage]", "[check-analyze-readonly]", "[check-guided-paths]", "[check-worktree-docs]", "[check-catalog-docs]", "[check-dependabot-config]", "[check-public-status]", "[check-evaluation]"} {
